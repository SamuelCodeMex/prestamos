...
<div class="container-fluid">
<?php 
//comprobamos id en bd si existe
    require_once "./controllers/UsuarioController.php";
    $insUsu =  new UsuarioController();
    $datosUs = $insUsu->datosUsuarioController("unico", $pagina[1]);
    if($datosUs->rowCount() == 1){
        $campos=$datosUs->fetch();
...
?>
    <form class="form-neon formularioAjax" action="<?php echo SERVERURL;?>ajax/usuarioAjax.php" method="POST" data-form="update" autocomplete="off">
                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <?php if($campos['usuario_estado'] == 0){echo '
                            ...
                            <select class="form-control" name="usuario_estado_up">
                                <option value="Activa"  <?php if($campos['usuario_estado'] == 0){echo 'selected=""';} ?>  >Activa</option>
                                <option value="Deshabilitada" <?php if($campos['usuario_estado'] != 0){echo 'selected=""';} ?>>Deshabilitada</option>
                            </select>
                        </div>
                    </div>
                    <?php } ?>
                    ...
                    <div class="col-12">
                        ...
                        <div class="form-group">
                            <select class="form-control" name="usuario_privilegio_up">
                                <option value="0"  <?php if($campos['usuario_privilegio'] == 0){echo 'selected=""';} ?>>Seleccione una opción</option>
                                <option value="1"  <?php if($campos['usuario_privilegio'] == 1){echo 'selected=""';} ?>>Control total</option>
                                <option value="2"  <?php if($campos['usuario_privilegio'] == 2){echo 'selected=""';} ?>>Edición</option>
                                <option value="3"  <?php if($campos['usuario_privilegio'] == 3){echo 'selected=""';} ?>>Registrar</option>
                            </select>
                        </div>
                    </div>
                </div>
        ...
        <?php if($lo->encryption($_SESSION['hmn_id']!= $pagina[1])){?>
            <input type="hidden" name="cuenta" value="impropia">
        <?php }else{?>
            <input type="hidden" name="cuenta" value="propia">
        <?php }?>
